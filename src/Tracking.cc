...
/**
 * 函数名称：Tracking 类的构造函数
 * 调用：在初始化System时调用
 * 功能：(1)初始化相机参数 (2) 初始化特征点提取器 (3) 设定远近点区分阈值 & 设定深度相机的因子
 * @param pSys  系统实例
 * @param pVoc  BOW字典
 * @param pFrameDrawer 帧绘制器
 * @param pMapDrawer  地图点绘制器
 * @param pMap 地图句柄
 * @param pKFDB 关键帧产生的词袋数据库
 * @param strSettingPath 配置文件路径(从此处读取相机参数)
 * @param sensor 传感器类型
 */
    ...
    mnLastRelocFrameId(0)                               //恢复为0,没有进行这个过程的时候的默认值
{
  ...
  // Step 1 从配置文件中加载相机参数 (Load camera parameters from settings file)
  ...
  // Step 2 加载ORB特征点有关的参数,并初始化3个特征点提取器
  // Load ORB parameters
  /// 每一帧提取的特征点数 1000
  ...
  // tracking 过程都会用到mpORBextractorLeft作为特征点提取器
  ...
  // 在传感器类型是STEREO或者RGBD时，计算用于区分近点还是远点的阈值
  if(sensor==System::STEREO || sensor==System::RGBD)
  {
    /// ThDepth 其实就是表示基线长度的多少倍,判断一个3D点远/近的阈值就是 mbf * 35 / fx
    /// 对于双目相机来说，z = f·b/d ,fx = n·f，由此 mThDepth 正比于  1/d ；
