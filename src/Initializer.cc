 ...
 * Step 4 计算得分比例来判断选取哪个模型来恢复位姿R,t
 *
                              ...
                              vector<bool> &vbTriangulated)
{
  ...
  // 记录的是帧2在帧1的特征点匹配索引 (预分配空间，大小和关键点数目一致mvKeys2.size())
  mvMatches12.clear();
  ...
  // step 1 : 准备数据
  // (1) 将以形参传入的关键点匹配关系vMatches12存储到初始化类的成员mvMatches12中
  // (2) 记录参考帧的每个关键点是否都有匹配关系了 mvbMatched1
  // (3) 新建vAllIndices存储特征点索引，存储特征点索引
  for( size_t i = 0, iend = vMatches12.size(); i < iend; i++ ){
    if(vMatches12[i] >= 0){
      // i表示帧1中关键点的索引值，vMatches12[i]的值为帧2的关键点索引值,没有对应关键点就是默认值-1
      mvMatches12.emplace_back(std::pair<int,int>(i,vMatches12[i]));
      // 标记参考帧1中的这个特征点有匹配关系
      mvbMatched1[i]=true;
    }
    else{
  ...
  // 新建一个容器vAllIndices存储特征点索引，并预分配空间 初始化所有特征点对的索引，索引值0到N-1
  std::vector<size_t> vAllIndices;
  vAllIndices.reserve(mvMatches12.size());
  ...
  // 在所有关键点对中为每次迭代生成8对关键点，用于估计H矩阵和F矩阵
  // 构造 mMaxIterations*8 的二维数组，初始化为0
  mvSets = vector<vector<size_t>>(mMaxIterations,
                                  vector<size_t>(8,0));
  // 用于进行随机数据样本采样，设置随机数种子
  DUtils::Random::SeedRandOnce(0);
  std::vector<size_t> vAvailableIndices; // 记录可以被选取点对
  // 开始每一次的迭代(200次迭代，每次随机选择八对关键点)
  for(int it = 0; it < mMaxIterations; it++)
  {
    vAvailableIndices = vAllIndices; /// 迭代开始的时候，所有的点都是可用的
    // Select a minimum set 选择最小的数据样本集，使用八点法求，所以这里就循环八次
  ...
  // step 3 : 计算Fundamental矩阵和Homography矩阵，为了加速分别开了线程计算
  // 用于标记在H和F的计算中哪些特征点对被认为是Inlier
  std::vector<bool> vbMatchesInliersH, vbMatchesInliersF;
  // score for H and F (计算出来的单应矩阵和基础矩阵的RANSAC评分，这里其实是采用重投影误差来计算的)
  float SH, SF;
  ...
  // (Wait until both threads have finished)等待两个计算线程结束,回收资源（阻塞）
  threadH.join();
  threadF.join();

  // Step 4 : Compute ratio of scores 计算得分比例来判断选取哪个模型来求位姿R,t
  ...
  if( RH>0.40 )
  {
    ...
    return ReconstructH(vbMatchesInliersH,	 // 输入，匹配成功的特征点对Inliers标记
                        ...
                        1.0,				 // 认为某对特征点的三角化测量中，认为其测量有效时,需要满足的最小视差角（如果视差角过小则会引起非常大的观测误差）,单位是角度
...
{
  // step 1: Normalize coordinates (归一化特征点坐标，使用T1 T2记录平移和尺度变换)
  ...
  // 记录各自的归一化矩阵: 把上述归一化的操作用矩阵来表示。这样特征点坐标乘归一化矩阵可以得到归一化后的坐标
  cv::Mat T1, T2;
  ...
  for(int it=0; it < mMaxIterations; it++)
  {
    // step 2 : (select a minimum set) 选择8个归一化之后的点对进行迭代
    for(size_t j=0; j<8; j++)
    {
    ...
    // step 5 : 利用重投影误差为当次RANSAC的结果评分
    currentScore = CheckHomography(H21i,
                                   H12i, 	          // 输入，单应矩阵的计算结果
                                   vbCurrentInliers, 	// 输出，特征点对的Inliers标记
                                   mSigma);				// 测量误差，在Initializer类对象构造的时候，由外部给定的

    // step 6 :更新具有最优评分的单应矩阵计算结果,并且保存所对应的特征点对的内点标记
...
void Initializer::FindFundamental(vector<bool> &vbMatchesInliers, float &score, cv::Mat &F21)
{
  ...
  for(int it=0; it<mMaxIterations; it++)
  {
    ...
    // Step 4 利用重投影误差为当次RANSAC的结果评分
    currentScore = CheckFundamental(F21i,
                                    vbCurrentInliers,
                                    mSigma);
 ...
 * 归一化后的点, in reference frame
 * 归一化后的点, in current frame
 * @param[in] vP1           参考帧中归一化后的特征点
 * @param[in] vP2           当前帧中归一化后的特征点
 ...
 * 代码使用了 Faugeras SVD-based decomposition 算法，参考文献
                               ...
                               int minTriangulated)
{
  ...
  // step 1 : 根据论文从H矩阵恢复R和t
  // NOTE : 目的 ：通过单应矩阵H恢复两帧图像之间的旋转矩阵R和平移向量T
  // Note: H矩阵分解常见有两种方法：Faugeras SVD-based decomposition 和 Zhang SVD-based decomposition
  // Note: 代码使用了 Faugeras SVD-based decomposition 算法，参考文献
  ...
  // step 2 : 通过三角化对 8 组解进行验证，并选择产生相机前方最多3D点的解为最优解
  for(size_t i = 0; i < 8; i++)
  {
    ...
    int nGood = CheckRT(vR[i],
                        ...
                        mvKeys1,mvKeys2,  // 特征点
                        mvMatches12,
                        vbMatchesInliers,            // 特征匹配关系以及Inlier标记
                        K,                              // 相机的内参数矩阵
...
/**
 * @brief Note: 用位姿来对特征匹配点三角化，从中筛选中合格的三维点
 *
 * @param[in] R                                     旋转矩阵R
 * @param[in] t                                     平移矩阵t
                          ...
                          float &parallax)
{
  ...
  // step 1 ：计算相机的投影矩阵(Projection Matrix) Camera 1: K[I|0] & Camera 2: K[R|t]
  ...
  // 计算第二个相机的投影矩阵,应先将世界坐标系的点转到第二个相机系，所以需要3D点需要先乘一个R_2_1 P2=K*[R|t]
  cv::Mat P2(3,4,CV_32F);
  R.copyTo(P2.rowRange(0,3).colRange(0,3));
  t.copyTo(P2.rowRange(0,3).col(3));
  P2 = K*P2;

  // 第二个相机的光心在世界坐标系下的坐标 O2
  // 第二个相机的光心在世界坐标系下的坐标 O2'(0,0,0)
  // T21 = [ R , t
  //         0 , 1 ]
  // T12 = T21_inv = [ RT , -RT*t
  //                    0 ,  1    ]
  ...
  for(size_t i=0, iend=vMatches12.size();i<iend;i++)
  {
    ...
    // 只要三角测量的结果(x,y,z)中有一个是无穷大的就说明三角化失败，跳过对当前点的处理，进行下一对特征点的遍历
