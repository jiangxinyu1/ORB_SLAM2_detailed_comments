 ...
 * @brief 给地图点添加观测信息
 *
 * Note ：记录该地图点可以被哪些KeyFrame的哪个特征点观测到，
 *  并更新该地图点被不同关键帧观测到的数目nObs，单目+1，双目或者rgbd+2
...
void MapPoint::AddObservation(KeyFrame* pKF, size_t idx)
{
  ...
  // Type : std::map<KeyFrame*,size_t> mObservations;
  if(mObservations.count(pKF))
  {
    // 如果该地图点已经被这个关键帧观测过，直接返回
    return;
  }

  // 如果该地图点没有被这个关键帧观测过，设置 key = 关键帧 ,value = 该地图点在关键帧中对应的特征点id
  mObservations[pKF]=idx;

  // 更新该关键帧被不同的关键帧观测的数目： 双目或者rgbd +2 , 单目 +1
